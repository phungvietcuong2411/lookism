...
import { chapters, getArcGroups, loadProgress } from '../../data/index';
import Donate from '../../assets/donate.jpg'

const Home = () => {
    const navigate = useNavigate();
    const savedChapterId = loadProgress();
    const arcGroups = getArcGroups();
    ...
    const handleSearch = () => {
        ...
        if (!num) return;
        const found = chapters.find((c) => c.id === num);
        if (!found) return;

        // If currently sorted desc and chapter is there, still jump
        setHighlightId(num);

        // Scroll after short delay so highlight renders first
        setTimeout(() => {
    ...
    return (
        <>
            <div className="bg-background-light min-h-screen mb-10">
                ...
                <div className="relative mb-8">
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-28 sm:-mt-40 relative z-10">
                        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-start">
                            {/* Cover */}
                            <div className="w-32 sm:w-48 md:w-56 flex-shrink-0 shadow-xl">
                                <img
                            ...
                            <div className="flex-1 pt-0 sm:pt-20">
                                ...
                                <div className="flex flex-wrap gap-6 items-center border-t border-slate-200 pt-4 mb-5">
                                    ...
                                    {savedChapterId && (
                                        <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest">
                                            Đang đọc:{' '}
                                            <span className="text-lg font-bold text-[#1754cf]">Chương {savedChapterId}</span>
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    <button
                                        onClick={() => navigate(`/chapter/${savedChapterId ?? latestChapter.id}`)}
                                    ...
                                    >
                                        {savedChapterId ? `Đọc tiếp (Chương ${savedChapterId})` : 'Đọc mới nhất'}
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                {/* ── Main body ── */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 px-4 sm:px-8 lg:px-36">
                    {/* Left column */}
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <section className="bg-white p-6 border border-slate-200 shadow-sm">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
                                <h3 className="text-xl font-bold flex-1">Danh sách chương</h3>
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {sortedChapters.map((chap) => {
                                            ...
                                            const isCurrent = savedChapterId === chap.id;
                                            return (
                                                <tr
                                                    key={chap.id}
                    ...
                    <div className="space-y-8">
                        ...
                        {/* Donate */}
                        <section className="bg-white p-6 border border-slate-200 shadow-sm flex flex-col items-center justify-center">
                            <h3 className="text-lg font-bold mb-6">NẠP TIỀN VÀO DONATE CHO TAO</h3>
                            <img src={Donate} alt="QR donate" />
                        </section>
                    </div>
                </div>
            </div>
